...
async def market_data(ticker_list):
    ...
    output_df.to_csv('market_data_synchronous.csv')
    print(f'Wrote data to \'market_data_synchronous.csv\' for {output_df.index.size} tickers (some may have blank data)')
    return output_df
...
pd.DataFrame().from_dict(status_codes, orient='index').to_csv('status_codes_synchronous.csv')

# Output list of errors
import csv
with open('errors_synchronous.csv', 'w', encoding="utf-8") as file:
